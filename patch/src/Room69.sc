				...
				(cond
					((or (Said 'look[<around][/!*]') (Said 'look/room,crypt'))
						(Print
							(Format ; "What an incredible place! This looks like an ancient burial tomb, with hieroglyphics written on the walls, and an old coffin against the far wall. %s %s"
								...
								0
								(if (== global170 0)
									{ערימת חבלים נמצאת בקצה המשטח העליון.}
								else
									{ }
								)
								(if (== ((Inv at: 4) owner:) 69) ; Pandora_s_Box
									{את רואה כאן תיבה קטנה ומוזרה למראה. זו בטח תיבת פנדורה.}
								else
									{ }
								)
							)
					...
					((Said 'look>')
						(cond
							...
							((or (Said '/box[<pandora]') (Said '/pandora[<box]'))
								(cond
					...
					((or (Said 'get/pandora') (Said 'get/chest') (Said 'get/box'))
						(cond
							((not local7)
					...
					((or (Said 'place,drop,return/box') (Said 'place,drop,return/pandora'))
						(cond
