...
(instance statusCode of Code
	...
	(method (doit param1)
		(Format param1 0 1 gScore gPossibleScore) ; "Score: %d of %d %s"
	)
)
...
(instance KQ4 of Game
	...
	(method (replay)
		...
		(TheMenuBar draw:)
		(SL enable:)
		(SetMenu 1283 110 (if (DoSound sndSET_SOUND) {כבה} else {הפעל}))
		(super replay:)
	)
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 temp3 [temp4 50])
		...
		(if (== (event type:) evSAID)
			...
			(cond
				...
				((Said 'talk/chicken')
					(if (gEgo has: 33) ; Magic_Hen
						(Print 0 62 #title {תרנגולת קסומה} #icon 431 0 0) ; "Cluck, cluck."
					else
						(DontHave) ; "You don't have it."
					)
				)
...
(instance Shovel of newInvItem
	...
	(method (showSelf)
		(Print
			...
			0
			#title
			(if (self loop:) {את חפירה שבור} else {את חפירה})
			#icon
			view
			loop
			cel
...
(instance Fishing_Pole of newInvItem
	...
	(method (showSelf)
		(Print
			...
			0
			#title
			(if (self loop:) {חכת דיג עם פיתיון} else {חכת דיג})
			#icon
			view
			loop
			cel
...
(instance Rose of newInvItem
	(properties
		said '/rose'
		description {איזה ורד אדום ויפהפה! מה זה?! בחיי, יש מפתח זהב קטן מחובר לוורד הזה!}
		owner 85
		view 409
	)
