...
def read_csv_file(csvdir):
    ...
    if warnings_duplicates:
        ...
        print('\n'.join([word[::-1] + "\t" + str(rooms) for word, rooms in warnings_duplicates]))
...
def write_vocab_file(entries, patchesdir, output_game_dir):
    ...
    for word in sorted(words.keys()):
        if word[0] != current_char:
            current_char = word[0]
            ord_index=ord(current_char)
            if ord_index > 1264:
                ord_index=ord_index-1264
            pointers[ord_index] = len(binary_vocab) - 2     # magic number is ignored
        entry = words[word]
        byte1 = entry['cls'] >> 4
