...
(procedure (localproc_0)
	(localproc_1)
	(Print &rest #title {טרול} #font 4 #at 16 20 #width 152 #dispose)
)
		...
		(switch (= state newState)
			(0
				...
				(if (IsFlag 2)
					...
					(SciAudio play: {trollAndGoat.mp3} -1)
				else
					;((ScriptID 0 23) number: 61 init: play:) ; backSound
					(SciAudio play: {troll.mp3} -1)
				)
				(SetFlag 13)
				(if (not (>= global101 1))
...
(instance bridge of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {גשר העץ הקטן והרעוע ישן מאוד, אך מוצק מספיק כדי לשאת פי כמה ממשקלך.}
	)
)
...
(instance tree1 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {הרבה עצים בוגרים וחינניים מצפים את גדות הנהר.}
	)
)
...
(instance smallTree of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {הו, תראו! זה עץ תינוק קטנטן!}
	)
)
...
(instance rocks of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {מספר סלעים רגילים יושבים בערימה כאן ליד גדת הנהר.}
	)
)
...
(instance rock2 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {צביר הסלעים הזה ליד שפת הנהר שולט על זרימת הנהר.}
	)
)
...
(instance bush3 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {שיח גדול ובריא יושב ליד הנהר.}
	)
)
...
(instance rRock of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {כמה סלעים בולטים מעל פני הנהר השוצף בפראות.}
	)
)
