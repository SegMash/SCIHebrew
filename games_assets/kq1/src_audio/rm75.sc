		...
		(cond
			...
			((or (Said '/rat>') (Said '//rat>') (Said '/*<rat>'))
				(cond
					...
					((Said 'feed>')
						(cond
							((or (Said '/cheese') (Said '/cheese/rat'))
...
(instance fightRat of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(6
				(EgoDeadNew
					{מכרסמת בך התחושה שזה היה טיפוס אפלולי מהסוג שלא היית אמור להתעסק איתו.}
				)
				(HandsOn)
			)
		)
