...
(instance witchToPot of Script
	...
	(method (handleEvent event)
		(cond
			...
			((Said 'move,pull,kill/witch[/caldron,brew,liquid]')
				(cond
					...
					(else
						...
						((ScriptID 0 21) stop:) ; gameSound
						(SciAudio stop:)
						(SciAudio play: {witchKilling.mp3} 0)
...
(instance table of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {זהו השולחן שבו המכשפה אוכלת את מזונה, מה שזה לא יהיה... או מי שזה לא יהיה!}
	)
...
(instance pot of NewFeature
	(properties
		...
		seeableDist 520
		shiftClick 369
		lookStr {הסיר חם מספיק כדי לשרוף את ידך. האש בטח בערה זמן רב.}
	)
