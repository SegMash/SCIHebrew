		...
		(cond
			...
			(
				...
				(if (not local1)
					...
					(SciAudio stop:)
					(SciAudio play: {witchInForest.mp3} -1)
...
(instance rock2 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {זהו אחד מבין סלעים רבים לא מעניינים המנקדים את רצפת היער.}
	)
)
...
(instance tree11 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {העצים האלה עקומים ומעוותים באופן גרוטסקי, והענפים נראים כאילו הם מושיטים יד לתפוס אותך.}
	)
)
...
(instance bush of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {שיחים וגפנים גדלים בערבוביה בחלק זה של היער.}
	)
)
