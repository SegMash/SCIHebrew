...
(instance rm79 of Rm
	...
	(method (handleEvent event &tmp temp0)
		(cond
			...
			((or (Said 'give/soup') (Said 'give/soup/man'))
				(cond
...
(instance wife of Prop
	(properties
		x 180
		y 116
		description {אשתו של חוטב העצים}
		view 181
	)
		...
		(switch (= state newState)
			(0
				...
				(if (not (IsFlag 113))
					...
					(SciAudio play: {woodcutter.mp3} -1)
				)
				(if (IsFlag 113)
					(wife loop: 1)
				)
...
(instance cupboard of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {יש ארון מטבח ישן ורעוע על הקיר.}
	)
)
...
(instance fireplace1 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {אש גדולה בוערת בעליצות באח, ומספקת קצת חום בבקתה הישנה והקרה.}
	)
)
...
(instance rug1 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {יש שטיח קטן ובלוי על הרצפה.}
	)
)
...
(instance nightstand of NewFeature
	(properties
		...
		seeableDist 320
		shiftClick 369
		lookStr {שידת הלילה הישנה אינה אלא שולחן קטן ופשוט למרגלות המיטה.}
	)
)
