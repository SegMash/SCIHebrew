...
(instance offCliff of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(EgoDeadNew {הניסיון שלך בקפיצת צוקים היה כישלון מוחץ.})
			)
		)
	)
)
...
(instance cliff1 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {צוקים תלולים מקיפים אגם הררי ציורי.}
	)
)
...
(instance lake1 of NewFeature
	(properties
		...
		seeableDist 320
		shiftClick 369
		lookStr {האגם ההררי הצלול מוקף בצוקים תלולים.}
	)
)
...
(instance rock1 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {סלע גדול ומשונן יושב בראש הצוק.}
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if (not global136)
					(EgoDeadNew
						{לאחר שחייה ארוכה, כוחך נחלש וידיך ורגליך מתעייפות. ככל שחייך חולפים לנגד עיניך, אתה מחליט...}
					)
				else
					(EgoDeadNew
						{אתה מתיז מים זמן מה, אבל למרבה הצער זה לא ישמור את ראשך מעל המים. כשאתה שוקע בפעם השלישית, תחושת שלווה שוטפת אותך...}
					)
				)
			)
		)
...
(instance tree1 of NewFeature
	(properties
		...
		shiftClick 369
		contClick 371
		lookStr {עץ רחב פרוש נראה כצללית על רקע השמש.}
	)
)
