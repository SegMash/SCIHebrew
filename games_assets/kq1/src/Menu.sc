...
(class MenuBar of MenuBar
	...
	(method (init)
		(AddMenu {\01} {אודות KQI`^a:עזרה`#1})
		(AddMenu
			{_קובץ_}
			{שמור משחק`#5:שחזר משחק`#7: --!:הפעל משחק מחדש`#9:יציאה^q}
		)
		(AddMenu
			{_פעולה_}
			{להתכופף`#4:לקפוץ `#6: --!:להשהות `^p:מלאי `^I:פקודה אחרונה`#3 }
		)
		(AddMenu { מהירות } { לשנות...`^s:--!: מהיר יותר`+: רגיל`=: איטי יותר`- })
		(AddMenu
			{_צלילים_}
			(if (DoSound sndSET_SOUND)
				{ גובה צליל...`^v: הפעל שמע`#2=1 }
			else
				{ גובה צליל...`^v: כבה שמע`#2=0 }
			)
		)
		(SetMenu 258 109 'aid[<!*][/!*]')
	...
	(method (handleEvent event &tmp temp0 temp1 temp2 [temp3 300])
		(switch (super handleEvent: event (User blocks:))
			...
			(1025
				(= temp1 6)
				(if (!= (= temp1 (GetNumber {מהירות (1 - 16)?} gSpeed)) -1)
			...
			(1282
				(if (GetMenu 1282 113)
					...
					(SetMenu 1282 113 0 110 {הפעל שמע})
				else
					(DoSound sndSET_SOUND 1)
					(SetMenu 1282 113 1 110 {כבה שמע})
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(SetMenu 769 113 1 110 {עמוד} 109 'stand')
			)
			(1
			...
			(2
				...
				(User canControl: 1)
				(SetMenu 769 113 0 110 {התכופף} 109 'duck')
				(self dispose:)
			)
		)
	)
