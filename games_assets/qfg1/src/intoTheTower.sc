...
(local
	...
	local4
	[local5 5] = [1 2 4 8 16]
	[local10 4] = [{פתיחה} {תפיסה} {חץ להבה} {התנעה}]
)
...
(instance intoTheTower of Script
	...
	(method (changeState newState &tmp temp0 [temp1 40])
		(switch (= state newState)
			...
			(2
				(Print 131 1 #at 5 40 #width 80 #mode 1 #title { ארסמוס } #dispose #time 3) ; "Come in and sit down."
			...
			(6
				...
				(Print 131 2 #at 5 40 #width 80 #mode 1 #title { ארסמוס } #dispose #time 4) ; "Fenrus, our guest has arrived."
				(= seconds 4)
			)
			(7
			...
			(15
				...
				(Print 131 4 #at 225 85 #width 90 #mode 1 #title { פנרוס } #dispose #time 3) ; "Ahhhhhh!"
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				((ScriptID 31 1) setCel: -1 setCycle: Fwd) ; wizard
				(gEgo setCel: 1)
				(Print 131 14 #at 5 40 #width 80 #mode 1 #title { ארסמוס } #dispose #time 5) ; "I so enjoy playing the Mage's Maze."
				(= seconds 5)
			)
			(5
				(gEgo setCel: 2)
