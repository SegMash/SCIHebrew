...
(instance birdieSings of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(Print 313 37 #at -1 15 #title {ציפור קטנה}) ; "CHIRP! CHIRP!"
				(Print 313 38 #at -1 15 #title {אישה זקנה וחביבה}) ; "Kitty, you'd better not be bothering the little birdie again!"
		...
		(switch (= state newState)
			...
			(11
				(if local14
					...
					(EgoDead 313 54 80 { איזה חתלתול חמוד! } 82 637 1 0) ; "When the Little Old Lady awakes to see what's going on, you have to concede to her (through lips that are as raw as hamburger) that you've been licked! She summons the Sheriff and his goon, Otto."
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(Print 313 59 #at -1 20 #mode 1 #title {אישה זקנה וחביבה}) ; "Help! Burglars! Sheriff! Help! Kitty! Kitty!"
				(= local14 1)
				(gEgo setScript: climbDown)
			)
		)
