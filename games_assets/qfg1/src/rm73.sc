...
(instance egoLoses of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				(EgoDead 73 62 82 516 2 5 80 {שמור על פיך סגור.}) ; "Done in by poisoned daggers and desperate desperadoes! Try to avoid annoying that deadly duo next time."
			)
		)
	)
)
...
(instance brutusThrows of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(9
				(EgoDead 73 64 82 516 2 5 80 {נדקר למוות.}) ; "Done in by daggers and a desperate desperado! Try to avoid annoying that deadly duo next time."
			)
		)
	)
)
