...
(procedure (proc997_0)
	(if (GetMenu 774 113)
		...
		(SetMenu 774 113 0 110 {הדלקת שמע})
	else
		(DoSound sndSET_SOUND 1)
		(SetMenu 774 113 1 110 {כיבוי שמע})
	)
)
...
(class MenuBar of MenuBar
	...
	(method (init) 
		(AddMenu { \01 } {אודות תהילה 1 `^g:עזרה`#1})
		(AddMenu
			{ קובץ }
			{שמור משחק`#5:לשחזר משחק`#7:--!:לאתחל משחק`#9:לצאת`^q}
		)
		(AddMenu
			{ משחק }
			{אנימציה מהירה`+:אנימציה רגילה`=:אנימציה איטית`-:--!:עוצמת שמע...`^v:כיבוי שמע`#2=1}
		)
		(AddMenu
			{ פעולה }
			{הטל לחש`^c:להלחם`^f:לברוח`^e:--!:להשהות משחק`^p:פעולה אחרונה`#3}
		)
		(AddMenu
			{ מידע }
			{מלאי`^I:גליון`^S:--!:יום/שעה`^T:שאל אודות`^a:הסתכל`^l}
		)
		...
		(switch (super handleEvent: event)
			...
			(773
				(if
					(!=
						(= temp3
							(GetNumber {?(1 - 16) עוצמה} (+ 1 (DoSound sndVOLUME)))
						)
						-1
					)
					(if (< (-- temp3) 0)
			...
			(1025
				(localproc_0 event {הטל })
			)
			(1026
				(localproc_0 event {להלחם })
			)
			(1027
				(localproc_0 event {לברוח })
			)
			(1029
			...
			(1285
				(localproc_0 event {שאל אודות })
			)
			(1286
				(localproc_0 event {להסתכל על })
			)
		)
	)
)
